...
import { ISellerGig } from '@quan0401/ecommerce-shared';
...
class GigService {
  ...
  async getAllRecordsOfIndex(index: string): Promise<AxiosResponse> {
    const response: AxiosResponse = await axiosGigInstance.get(`/all/${index}`);
    return response;
  }
