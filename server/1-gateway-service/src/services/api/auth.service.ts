...
import { IAuth } from '@quan0401/ecommerce-shared';
...
class AuthService {
  ...
  async getRefreshToken(email: string): Promise<AxiosResponse> {
    const response: AxiosResponse = await axiosAuthInstance.get(`/refresh-token/${email}`);
    return response;
  }
  ...
  async getGigs(from: string, size: string, type: string, query: string): Promise<AxiosResponse> {
