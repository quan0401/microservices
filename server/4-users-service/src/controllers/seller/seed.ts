...
import { BadRequestError, IBuyerDocument, IEducation, IExperience, ISellerDocument } from '@quan0401/ecommerce-shared';
import { Request, Response } from 'express';
import { createSeller, getRandomSellers, getSellerByEmail, getSellerById, getSellerByUsername } from '~/services/seller.service';
import { v4 as uuidv4 } from 'uuid';
import { getRandomBuyers } from '~/services/buyer.service';
import { floor, random, sample, sampleSize } from 'lodash';

export const seedSellers = async (req: Request, res: Response): Promise<void> => {
  ...
  for (let i = 0; i < buyers.length; i++) {
    ...
    if (checkIfSellerExist) {
      throw new BadRequestError('Seller already exist.', 'seedSellers method');
    }
