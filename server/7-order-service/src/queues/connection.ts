...
const log: Logger = winstonLogger(config.ELASTIC_SEARCH_URL!, 'orderServiceQueueConnection', 'debug');
...
export async function createConnection(): Promise<Channel | undefined> {
  try {
    ...
    log.info('Orderer server connected to queue successfully...');
    closeConnection(channel, connection);
    return channel;
  } catch (error) {
    log.error('Connection to amqplib failed. Try again...');
    log.log('error', 'orderServiceQueueConnection createConnection() error.', error);
  }
  return undefined;
}
