import {
  ...
  lowerCase
} from '@quan0401/ecommerce-shared';
...
export const createOrder = async (data: IOrderDocument): Promise<IOrderDocument> => {
  ...
  await sendNotification(order, data.sellerEmail, 'placed an order for your gig.');
  return order;
};
...
export const cancelOrder = async (orderId: string, data: IOrderMessage): Promise<IOrderDocument> => {
  ...
  sendNotification(order, order.sellerEmail, 'cancelled your order delivery.');
  return order;
};
...
export const approveOrder = async (orderId: string, data: IOrderMessage): Promise<IOrderDocument> => {
  ...
  sendNotification(order, order.sellerEmail, 'approved your order delivery.');
  return order;
};
...
export const sellerDeliverOrder = async (orderId: string, delivered: boolean, deliveredWork: IDeliveredWork): Promise<IOrderDocument> => {
  ...
  if (order) {
    ...
    sendNotification(order, order.buyerEmail, 'delivered your order.');
  }
  return order;
};
...
export const requestDeliveryExtension = async (orderId: string, data: IExtendedDelivery): Promise<IOrderDocument> => {
  ...
  if (order) {
    ...
    sendNotification(order, order.buyerEmail, 'requested for an order delivery date extension.');
  }
  return order;
};
...
export const approveDeliveryDate = async (orderId: string, data: IExtendedDelivery): Promise<IOrderDocument> => {
  ...
  if (order) {
    ...
    sendNotification(order, order.sellerEmail, 'approved your order delivery date extension request.');
  }
  return order;
};
...
export const rejectDeliveryDate = async (orderId: string): Promise<IOrderDocument> => {
  ...
  if (order) {
    ...
    sendNotification(order, order.sellerEmail, 'rejected your order delivery date extension request.');
  }
  return order;
};
...
export const updateOrderReview = async (data: IReviewMessageDetails): Promise<IOrderDocument> => {
  ...
  sendNotification(order, data.type === 'buyer-review' ? order.sellerEmail : order.buyerEmail, `left you a ${data.rating} star review`);
  return order;
};
