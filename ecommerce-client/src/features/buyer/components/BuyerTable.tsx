                ...
                return (
                  <tr key={uuidv4()} className="border-grey mb-2 flex flex-col flex-nowrap border-b bg-white sm:mb-0 sm:table-row ">
                    ...
                    <td className="px-3 py-1 text-left lg:p-3 lg:text-center">
                      {/* <span
                        className={`status rounded bg-transparent p-0 text-xs font-bold uppercase text-black sm:px-[5px] sm:py-[4px] ${lowerCase(order.status.replace(/ /g, ''))}`}
                      >
                        {order.status}
                      </span> */}

                      <span
                        className={`status rounded border bg-green-700  text-white p-0 text-xs font-bold uppercase sm:px-[5px] sm:py-[4px] ${lowerCase(order.status.replace(/ /g, ''))}`}
                      >
                        {order.status}
                      </span>
                    </td>
