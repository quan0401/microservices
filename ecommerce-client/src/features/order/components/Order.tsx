  ...
  useEffect(() => {
    socket.on('order notification', (order: IOrderDocument) => {
      if (order.orderId === orderId) {
        setOrder({ ...order } as IOrderDocument);
      }
    });
  }, [orderId]);
    ...
    <div className="container mx-auto">
      <div className="flex flex-wrap">
        <div className="order-last w-full p-4 lg:order-first lg:w-2/3">
          ...
          {order && order.buyerEmail === authUser.email && (
            <div className="mt-4 flex flex-col justify-between bg-white md:flex-row">
              ...
              <div className="mb-4 ml-5 w-full justify-center self-center text-left md:mr-3 md:w-1/3 md:text-right">
                {order && order.delivered && order.buyerEmail === authUser.email && (
                  <Button
        ...
        <div className="w-full p-4 lg:w-1/3 ">
          {Object.keys(order).length > 0 ? (
            <>
              {order.delivered && authUser.email === order.sellerEmail && <DeliveryTimer order={order} authUser={authUser} />}
              {order.delivered && authUser.email === order.sellerEmail && <></>}
              ...
              <div className="bg-white">
                <div className="mb-2 flex flex-col border-b px-4 pb-4 pt-3 md:flex-row">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span
                      className={`status text-center mt-1 w-24 bg-blue-400 rounded px-[3px] py-[3px] text-xs font-bold uppercase text-white md:ml-4 ${order.status.replace(
                        / /g,
                        ''
                      )}`}
                    >
