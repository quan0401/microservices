...
const NotificationDropdown: FC<IHomeHeaderProps> = ({ setIsNotificationDropdownOpen }): ReactElement => {
  ...
  const { data, isSuccess } = useGetNotificationsByIdQuery(`${authUser.email}`, { refetchOnMountOrArgChange: true });
              ...
              <div className="flex px-4">
                <div className="flex-shrink-0 mt-1">
                  <img
                    ...
                    src={data.senderEmail === authUser?.email ? data.receiverPicture : data.senderPicture}
                    alt=""
                  />
                </div>
                <div className="w-full pl-3 pt-2">
                  <div className="text-[13px] font-normal leading-4 flex justify-between">
                    <div className="font-normal w-[85%]">
                      <span className="font-bold pr-1">{data.senderEmail === authUser?.email ? data.receiverEmail : data.senderEmail}</span>
                      {data.message}
                    </div>
